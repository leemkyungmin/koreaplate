...
.
.calendar_body thead, .calendar_body thead tr {
	width: 100%;
}
...
<body>
	...
	<form name="calendarFrm" id="calendarFrm" method="GET">
		...
		<div class="top_title">${deptDTO.dName} 예약</div>
		...
		<div class="calendar">
			<div class="navigation">
				...
				<span class="this_month"> &nbsp;${today_info.search_year}. <c:if
					...
					href="calendar?year=${today_info.after_year}&month=${today_info.after_month}&cNo=${cDTO.cNo}&dSaup_no=${deptDTO.dSaup_no}">&gt;</a>
			</div>
			<script type="text/javascript">
			...
			<table class="calendar_body">
				...
				<tbody>
					<tr class="appointBtn">
						<c:forEach var="dateList" items="${dateList}"
							...
							<c:choose>
								<c:when
									test="${dateList.value eq 'notButton' and date_status.index%7==6}">
									<td id="${dateList.date}" class="dayCSS">
										<div class="notButton">${dateList.date}</div>
									</td>
								</c:when>
								<c:when
									test="${dateList.value eq 'notButton' and date_status.index%7==0}">
					</tr>
					<tr>
						<td id="${dateList.date}" class="dayCSS">
							<div class="notButton">${dateList.date}</div>
						...
						<c:when
							test="${dateList.value eq'today' and date_status.index%7==0}">
					</tr>
					<tr>
						<td id="${dateList.date}" class="dayCSS"
							onclick="fn_a(${dateList.date})">
						...
						<c:when
							test="${dateList.value eq 'Button' and date_status.index%7==6}">
							<td id="${dateList.date}" class="dayCSS"
								onclick="fn_a(${dateList.date})">
						...
						<c:when
							test="${dateList.value eq 'Button' and date_status.index%7==0}">
					</tr>
					<tr>
						<td id="${dateList.date}" class="dayCSS"
							onclick="fn_a(${dateList.date})">
				...
				<tfoot>
					<tr>
						<td colspan="7" class="notice">※ 예약을 원하시는 날짜를 클릭하세요.</td>
					</tr>
				</tfoot>
			</table>
		</div>
			...
			<div class="custInfo">
				<table class="appointment_table">
					<tr>
						...
						<td class="text_desc"><input class="custInfo_textbox"
							type="text" name="" value="${cDTO.cName}" size="13" readonly /></td>
					</tr>
					<tr>
						<td class="text_subject">연락처 :</td>
						<td class="text_desc"><input class="custInfo_textbox"
							type="text" name=""
							value="${fn:substring(cDTO.cPhone, 0, 3)}-${fn:substring(cDTO.cPhone, 3, 7)}-${fn:substring(cDTO.cPhone, 7, 11)}"
							size="13" readonly /></td>
					</tr>
				</table>
			</div>
			<div class="appointInfo">
				<table class="appointment_table">
					<tr>
						...
						<td class="text_desc"><input id="aDate_day_textbox"
							type="text" class="text_type1" readonly /> <input
							id="aDate_day_hidden" type="hidden" name="aDate_day" /></td>
					</tr>
					<tr>
						<td class="text_subject">시간 :</td>
						<td class="text_desc remainPerson"></td>
					</tr>
					<tr>
						<td class="text_subject">인원 :</td>
						<td class="text_desc selectAp_count"><input
							id="aP_count_textbox" type="text" name="aP_count"
							placeholder="숫자" size="1" /> 명</td>
					</tr>
				</table>
			</div>
			<div class="submit_btn_wrap">
				<input class="submit_btn" type="button" value="예약하기"
					onclick="aP_countCHK(this.form)" /> <input type="hidden" name="cNo"
					value="${cDTO.cNo}" /> <input type="hidden" name="dSaup_no"
					value="${deptDTO.dSaup_no}" />
			</div>
		</div>
	</form>
