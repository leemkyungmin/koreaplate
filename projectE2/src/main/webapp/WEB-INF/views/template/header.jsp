...
<link rel="stylesheet" href="resources/assets/style/main.css"
	type="text/css">
<link rel="stylesheet" href="resources/assets/style/nav.css"
	type="text/css">
<link rel="stylesheet"
...
<body class="pt-5">
	...
	<header class="Header" data-page="home">
		<ul class="nav-wrap">
			<li class="nav-logo-wrap"><a href="index"><img alt="로고이미지"
					src="<c:url value="/resources/assets/images/logo01.png" />"
					class="logo"></a></li>
			<li class="nav-search-wrap">
				<form class="nav-search-form" action="searchPage">
					...
					<input id="search_keyword" class="nav-HomeSearchInput"
						name="main-search" type="text" maxlength="50"
						placeholder="지역 또는 맛집 이름" autocomplete="off"> <input
						class="nav-search-submit" type="submit" value="전송">
				</form>
			</li>
			<c:if test="${sessionScope !=null && sessionScope.cGrede==5 }">
				<li class="nav-list-wrap"><a href="adminmanagePage"
					class="nav-btn">관리자 페이지로 이동하기</a></li>
			</c:if>

			<li class="nav-list-wrap"><a href="main2" class="nav-btn">맛집
					리스트</a></li>

			<li class="nav-user-wrap"><c:if
					test="${sessionScope.cId eq null and sessionScope.dId eq null }">
					<a href="loginChoicePage" class="nav-btn"><i
						class="far fa-user"></i></a>
					<!-- 로그인 안돼있으면 로그인페이지 돼어있으면 마이페이지 이동. -->
				</c:if> <c:if test="${sessionScope.cId ne null}">
					<a href="myPage" class="nav-btn"><i class="far fa-user"></i></a>
					<input type="button" value="로그아웃" onclick="fn_logout()" />

				</c:if> <c:if test="${sessionScope.dId ne null }">
