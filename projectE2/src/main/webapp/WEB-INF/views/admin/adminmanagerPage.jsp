								...
								<div class="chart-area" style="height: 90%;">
									...
									<div id="custom" style="width: 1000px;">
										<form action="searchQueryCusInfo">
											<div class="center">
												<span class="totalCustomer" style="margin: auto;">전체
													회원 수 : ${totalRecord}명</span>
												<div class="searchbox_wrap">
													<input type="text" class="searchbox" name="query"
														placeholder="이름/아이디" size="15" /> <input type="submit"
														class="searchbox" value="검색" />
												</div>
											</div>
										</form>
										<form id="myForm" method="post">
												...
												<tbody>
													...
													<c:if test="${not empty cList}">
														<c:forEach var="user" items="${cList }">
															<tr>
																<td><input type="checkbox" id="cNo" name="cNo"
																	value="${user.cNo}" /></td>
																...
																<td>${user.cEmail }</td>
																<td><c:if test="${user.cGrade ==1 }">브론즈</c:if> <c:if
																		test="${user.cGrade ==2 }">실버</c:if> <c:if
																		test="${user.cGrade ==3 }">다이아</c:if> <c:if
																		test="${user.cGrade ==4 }">vip</c:if> <c:if
																		test="${user.cGrade ==5 }">관리자</c:if></td>
											...
											<div class="button_wrap">
												<input type="button" id="deleteBtn" value="회원삭제"
													onclick="deleteCust(this.form)" />
											</div>
										</form>
										<div class="pageView">
											<!-- 페이지 뷰 -->
